...
#include "AP_Ticommunication_Serial.h"
...
AP_Ticommunication_Serial::AP_Ticommunication_Serial(AP_Ticommunication &_frontend):
    AP_Ticommunication_Backend(_frontend)
{
    //internal_state.estimated_consumed_fuel_volume_cm3 = 0; // Just to be sure
    port = AP::serialmanager().find_serial(AP_SerialManager::Serialprotocol_Ticommunication, 0);     // Check which port is set to Ticommunication
}


void AP_Ticommunication_Serial::update()
{
    if (!port) {
        return;
    }
